...
const InstructionsSection: React.FC<InstructionsSectionProps> = ({sectionFields,templatePrintEdit,sectionName,setChangedPopoverValue,templateHeaderGlobal,Handlers,key,onlineChar}) => {
  const instructionsObj: any = sectionFields.find((item) => item.fieldKey === 'instructions');
  ...
  useEffect(() => {
    if(textAreaOnchange){
      ...
      if(textAreaOnchangeReplace){
        if (window.location.href.includes('onlineAssesment/printforpreview') && instructionsObj === undefined) {
          setChangedPopoverValue({ sectionName: "instructionsSection", fieldKey: "instructions", value: textAreaOnchangeReplace })
        } else {
  ...
  return (
    <div className={`InstructionsSectionBlock`}>
      {(window.location.href.includes('onlineAssesment/printforpreview') || instructionsObj) && (
        <>
        {!templatePrintEdit ?
          <>
          ...
          : <div className={`${templatePrintEdit ? "instructionsSect w-100" : ""}`}>
            ...
            <InstructionRTE registerName={'instructions'} setTextAreaOnchange={setTextAreaOnchange} characterLimit={onlineChar ? onlineChar : 4000} handlers={Handlers} placeHolder={"Start typing instructions..."} fieldsRequired={["language","common","attachment","formula"]} key={key}/>
            </div>}
        </>
      )}
    </div>
