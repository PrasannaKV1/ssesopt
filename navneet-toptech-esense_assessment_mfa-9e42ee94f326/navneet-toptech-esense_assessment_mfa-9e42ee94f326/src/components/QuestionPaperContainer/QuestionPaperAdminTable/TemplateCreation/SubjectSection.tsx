...
const SubjectSection: React.FC<SubjectSectionProps> = ({  subjects, selSubject, templatePrintEdit, setAnchorEl, setChangedPopoverValue, sectionName }) => {
    ...
    <div className={`SubjectSectionBlock SubjectSectionBlock${sectionFieldAsOrdered[0].fieldSequence}`}>
      {sectionFieldAsOrdered.map((item: sectionFieldsItem) =>
        item.fieldKey === 'subject' ? (
          ...
          :
                <div className={`headerTextEdit d-flex ${!item.fieldSelected ? 'hidden' : ''}`}
                  onClick={(event) => {
                    if (item?.fieldKey == 'totalMarks' && !isOnlineAssesssment) {
                      setAnchorEl(event.currentTarget);
                      setChangedPopoverValue({ sectionName: sectionName, fieldKey: item.fieldKey, value: item.fieldName })
                    } else {
